function crash = checkCrash(car, obstacle_polygon, obstacle_circle)
    
    %% ----- Initialization

    A = get(car(1),'Vertices');     %Get vertices of the vehicle
    dist_crash = 5;               %Sensor distance (1 = 10cm) for crash
    obs_crash = false;              %Obstacle crash
    sens_crash = false;             %Crash sensor values are below 1.
...
%     sensor = 10.*sensor;                % Scale the sensor values with 10
    crash = obs_crash;    % Store the results in a vector
end
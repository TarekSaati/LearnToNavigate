...
obs1 = obs1(1:obj_stp:end,:);   N1 = length(obs1(:,1));
obs2 = obs2(1:obj_stp:end,:);   N2 = length(obs2(:,1));
c1 = cog(obs1); c2 = cog(obs2);
cog_dist = dis(c1, c2, 'p');    wght =  60/map_wd;
%%
l1 = zeros(1,N1+4);
for i=1:N1+4
...
for i=1:Ns1
    for j=1:Ns2
        ...
        if J < 0.006*(cog_dist*wght)^1.2 && isempty(idx1)
            xi = ppval(X1,l1(5)+i*lstp); yi = ppval(Y1,l1(5)+i*lstp);
            xj = ppval(X2,l2(5)+j*lstp); yj = ppval(Y2,l2(5)+j*lstp);
            mij = (yj-yi)/(xj-xi);
            if abs(2*atan(mij)-atan(m2(j))-atan(m1(i))) < 0.3
                idx1 = [idx1;i];
                idx2 = [idx2;j];
            end
        end
        if ~isempty(idx1) && J < 0.006*(cog_dist*wght)^1.2
            ...
            mij = (yj-yi)/(xj-xi);
            if  M > 0.2 && abs(2*atan(mij)-atan(m2(j))-atan(m1(i))) < 0.3
                idx1 = [idx1;i];
                idx2 = [idx2;j];
            elseif  M < 0.2 && min_dist*wght > 5 && abs(2*atan(mij)-atan(m2(j))-atan(m1(i))) < 0.3
                idx1 = [idx1;i];
                idx2 = [idx2;j];
            end
        end
...
pnts1 = [ppval(X1,l1(5)+idx1*lstp) ppval(Y1,l1(5)+idx1*lstp)]; 
pnts2 = [ppval(X2,l2(5)+idx2*lstp) ppval(Y2,l2(5)+idx2*lstp)];
s = linspace(0,1);
for i=1:length(pnts1(:,1))
    plot(pnts2(i,1)+s*(pnts1(i,1)-pnts2(i,1)), pnts2(i,2)+s*(pnts1(i,2)-pnts2(i,2)));
    hold on;
end
plot(obs1(:,1),obs1(:,2),'b.'); plot(obs2(:,1),obs2(:,2),'r.');
plot(ppval(X1,span1),ppval(Y1,span1),'b-'); plot(ppval(X2,span2),ppval(Y2,span2),'r-');
plot(pnts1(:,1),pnts1(:,2),'b*'); plot(pnts2(:,1),pnts2(:,2),'r*');
axis equal
